...
def test_LocalFeedDataRepo():
    ...
    try:
        ...
        r.metadata.download_result = DownloadOperationResult(started=ts, status=FeedDownloader.State.in_progress.value, results=[])
        r.flush_metadata()
        r.metadata = None
        r.reload_metadata()
        assert r.metadata.download_result.started == ts
        assert r.metadata.download_result.status == FeedDownloader.State.in_progress.value

        r.write_data('feed1', 'group1', chunk_id=0, data=b'{"next_token": "something", "data": [{"somekey": "somevalue"}]}')
