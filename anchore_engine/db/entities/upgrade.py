...
def remove_policy_engine_sizes():
    ...
    for table_name, column_name in to_update:
        log.err('Updating table {} column {} to varchar'.format(table_name, column_name))
        try:
            rc = engine.execute('ALTER TABLE {} ALTER COLUMN "{}" type varchar'.format(table_name, column_name))
        except Exception as ex:
            log.err('Failed updating {} column {} type to varchar. Err: {}'.format(table_name, column_name, str(ex)))
            raise ex
