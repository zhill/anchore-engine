...
class FeedServiceClient(IFeedSource):
                ...
                if record['success']:
                    ...
                    if data and 'feeds' in data:
                        feed_list.feeds.extend([
                            FeedAPIRecord(name=x.get('name'), description=x.get('description'), access_tier=x.get('access_tier')) for x in data['feeds']])
    ...
    def list_feed_groups(self, feed: str) -> FeedGroupList:
        ...
        while more_data:
            url = self.group_url.format(feed=feed) + (('?next_token=' + next_token) if next_token else '')
                ...
                if record['success']:
                    ...
                    if 'groups' in data:
                        group_list.groups.extend([
                            FeedAPIGroupRecord(name=x.get('name'), description=x.get('description'), access_tier=x.get('access_tier')) for x in data['groups']])
                    if 'next_token' in data and data['next_token']:
                        next_token = data['next_token']
                        more_data = True
                    else:
    ...
    def get_feed_group_data(self, feed: str, group: str, since: datetime.datetime = None, next_token: str = None):
        try:
            record = self.get_raw_feed_group_data(feed, group, since, next_token)
            if record['success']:
                next_token, group_data, count = self._extract_response_data(record['content'])
                return GroupData(data=group_data, next_token=next_token, since=since, record_count=count)
            else:
                raise Exception(
                    'Feed list operation failed. Msg: {}. Response: {}'.format(record.get('err_msg'),
                                                                               record.get('text')))
    ...
    def get_raw_feed_group_data(self, feed: str, group: str, since: datetime.datetime = None, next_token: str = None) -> typing.Tuple:
        ...
        try:
            return self.http_client.execute_request(requests.get, url, retries=self.retry_count)
        except Exception as e:
            logger.debug('Error executing feed data download: {}'.format(e))
            raise e
...
def get_client(feeds_url=None, user=None, conn_timeout=None, read_timeout=None, ssl_verify=None):
    """
    ...
    logger.debug('Initializing a feeds client: url={}, user={}, conn_timeout={}, read_timeout={}'.format(feeds_url, user if user is None or type(user) not in [tuple, list] or len(user) == 0 else (user[0], '***redacted**'), conn_timeout, read_timeout))
