...
class LocalFeedDataRepo(object):
    ...
    def read(self, feed, group, start_index):
        ...
        files = [os.path.join(self.root_dir, feed, group, str(x)) for x in sorted(int(y) for y in os.listdir(os.path.join(self.root_dir, feed, group)))]
...
class FileListJsonIterator(object):
    ...
    def _init_itr(self):
        if self.current_file_index < len(self.files):
            ...
            logger.info('Reading feed data from file {}'.format(self.files[self.current_file_index]))
            self.file_itr = iter(FeedDataFileJsonIterator(self.files[self.current_file_index]))
        else:
            self.file_itr = None
            raise StopIteration()
...
def _download_start_metadata() -> DownloadOperationResult:
    return DownloadOperationResult(started=datetime.datetime.utcnow(),
...
def _group_download_start_metadata(group: GroupDownloadOperationConfiguration):
    return GroupDownloadResult(feed=group.feed,
                               group=group.group,
                               total_records=0,
                               status=FeedDownloader.State.in_progress.value,
...
def _update_download_failed(conf: GroupDownloadResult, record_count: int) -> GroupDownloadResult:
...
def _update_download_complete(conf: GroupDownloadResult, record_count: int) -> GroupDownloadResult:
...
class FeedDownloader(object):
    ...
    def __init__(self, download_root_dir: str, config: DownloadOperationConfiguration, client: IFeedSource, fetch_all: bool = False):
        """
        ...
        logger.debug('Initializing downloader for operation {}. Will write to path: {}'.format(config.uuid, op_dir))
        repo_meta = LocalFeedDataRepoMetadata(download_configuration=config, data_write_dir=op_dir)
    ...
    def execute(self, feed_name=None, group_name=None) -> LocalFeedDataRepo:
        ...
        try:
            ...
            logger.debug_exception("Could not initialize the feed data download location: {}. Failing fetch attempt".format(self.local_repo.root_dir))
            raise

        groups_failed = 0
        try:
            for group in self.config.groups:
                if feed_name and group.feed != feed_name or (group_name and group_name != group.group):
                    # Skip groups that don't match if a specific group was requested
                    logger.debug('Download configuration has record for group {}/{} but only {}/{} requested, so skipping'.format(group.feed, group.group, feed_name, group_name))
                    continue
                ...
                try:
                    ...
                    logger.info('Downloading data for group {}/{}'.format(group.feed, group.group))
                    with timer('data download for group {}/{}'.format(group.feed, group.group), log_level='info'):
                    ...
                    logger.exception('Error downloading data for group {}/{}'.format(group.feed, group.group))
            ...
            if groups_failed > 0:
                self.local_repo.metadata.download_result.status = FeedDownloader.State.failed.value
            else:
                self.local_repo.metadata.download_result.status = FeedDownloader.State.complete.value
        except:
            logger.debug_exception('Error fetching feed data, setting status to failed for operation {}'.format(self.config.uuid))
            self.local_repo.metadata.download_result.status = FeedDownloader.State.failed.value
            raise
        finally:
    ...
    def _fetch_group_data(self, group: GroupDownloadOperationConfiguration) -> typing.Iterable:
        """
        ...
        while get_next:
            logger.info('Downloading page {} of feed data for feed group: {}/{}'.format(chunk_number, group.feed, group.group))
            group_data = self.service_client.get_feed_group_data(group.feed, group.group, since=since, next_token=next_token)
            ...
            if group_data.data is not None:
                self.local_repo.write_data(group.feed, group.group, chunk_number, ensure_bytes(group_data.data))
            chunk_number += 1
            yield group_data.record_count

        logger.info('Completed data download of for feed group: {}/{}. Total pages: {}'.format(group.feed, group.group, chunk_number))
