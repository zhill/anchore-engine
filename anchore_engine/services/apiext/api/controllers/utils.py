...
def validate_tag_source(tag_source: dict, api_schema: dict) -> bool:
    ...
    :return:
    """
    schema = api_schema.get('definitions', {}).get(REGISTRY_TAG_SOURCE_SCHEMA_DEFINITION_NAME)

    try:
        jsonschema.validate(tag_source, schema)
    except jsonschema.ValidationError as e:
        raise BadRequest('Validation error', detail={'validation_error': '{}'.format(e)})

    if not validate_pullstring_is_tag(tag_source['pullstring']):
        raise BadRequest('Must have tag-based pull string', detail={"invalid_value": tag_source['pullstring']})


def validate_digest_source(digest_source: dict, api_schema: dict) -> bool:
    schema = api_schema.get('definitions', {}).get(REGISTRY_DIGEST_SOURCE_SCHEMA_DEFINITION_NAME)
    ...
    if not validate_pullstring_is_digest(digest_source['pullstring']):
        raise BadRequest('Must have digest-based pull string', detail={"invalid_value": digest_source['pullstring']})
    if not validate_pullstring_is_tag(digest_source['tag']):
        raise BadRequest('Must have tag-based pull string', detail={"invalid_value": digest_source['tag']})


def validate_archive_source(archive_source: dict, api_schema) -> bool:
        schema = api_schema.get('definitions', {}).get(REGISTRY_ARCHIVE_SOURCE_SCHEMA_DEFINITION_NAME)
    ...
    if digest:
        if DIGEST_REGEX.match(digest) is not None:
            ...
            if tag:
                ...
                digest_pullstring = parsed['registry'] + '/' + parsed['repo'] + '@' + digest
            else:
                raise ValueError('For a digest-based analysis, the tag property must also be populated')
        else:
        ...
        raise BadRequest('Must include either "tag", "tag" and "digest", or "source" property in body', detail={})
...
def validate_image_add_source(analysis_request_dict, api_schema):
    ...
    if source:
        if top_digest is not None or top_tag is not None or top_dockerfile is not None:
            raise BadRequest('Cannot use both source property and tag, digest, or dockerfile property at top level', detail={})
