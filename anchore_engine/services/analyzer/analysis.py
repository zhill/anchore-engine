...
from anchore_engine.services.analyzer.utils import fulltag_from_detail, emit_events, update_analysis_started, update_analysis_complete, update_analysis_failed
...
def import_to_policy_engine(account: str, image_id: str, image_digest: str):
    ...
    try:
        logger.debug("clearing any existing image record in policy engine: {} / {} / {}".format(account, image_id, image_digest))
...
def analysis_sucess_metrics(analysis_time: float, allow_exception=False):
    try:
        anchore_engine.subsys.metrics.counter_inc(name='anchore_analysis_success')
        anchore_engine.subsys.metrics.histogram_observe('anchore_analysis_time_seconds', analysis_time, buckets=ANALYSIS_TIME_SECONDS_BUCKETS, status="success")
    except:
        if allow_exception:
            raise
        else:
            logger.exception('Unexpected exception during metrics update for a successful analysis. Swallowing error and continuing')
...
def analysis_failed_metrics(analysis_time: float, allow_exception=False):
    try:
        ...
        anchore_engine.subsys.metrics.histogram_observe('anchore_analysis_time_seconds', analysis_time, buckets=ANALYSIS_TIME_SECONDS_BUCKETS, status="fail")
    except:
        if allow_exception:
            raise
        else:
            logger.exception('Unexpected exception during metrics update for a failed analysis. Swallowing error and continuing')


def store_analysis_results(account: str, image_digest: str, image_record: dict, analysis_result: list, image_manifest: dict, analysis_events: list, image_content_types: list):
    """
    ...
    try:
        ...
        if not rc:
            ...
            raise CatalogClientError(msg='Catalog client returned failure', cause='Invalid response from catalog API - {}'.format(str(rc)))
    except Exception as e:
        err = CatalogClientError(msg='Failed to upload analysis data to catalog', cause=e)
        event = events.SaveAnalysisFailed(user_id=account, image_digest=image_digest, error=err.to_dict())
        analysis_events.append(event)
    ...
    try:
        ...
        for content_type in image_content_types:
            try:
                image_content_data[content_type] = helpers.extract_analyzer_content(analysis_result, content_type, manifest=image_manifest)
            except Exception as err:
                logger.warn("ERR: {}".format(err))
                image_content_data[content_type] = {}
        ...
        if image_content_data:
            ...
            logger.debug("adding image analysis data to image_record")
            helpers.update_image_record_with_analysis_data(image_record, analysis_result)
