...
def perform_analyze(
    ...
    return perform_analyze_nodocker(
        userId,
        manifest,
        image_record,
        registry_creds,
...
def process_analyzer_job(system_user_auth, qobj, layer_cache_enable):
    ...
    try:
        ...
        logger.info(
            "image dequeued for analysis: " + str(userId) + " : " + str(imageDigest)
        )
        if image_record[
            "analysis_status"
        ...
        try:
            ...
            image_record[
                "analysis_status"
            ] = anchore_engine.subsys.taskstate.working_state("analyze")
            ...
            try:
                ...
                event = events.ImageAnalysisFailed(
                    user_id=userId, image_digest=imageDigest, error=e.to_dict()
                )
            ...
            logger.info(
                "adding image analysis data to catalog: userid={} imageId={} imageDigest={}".format(
                    userId, imageId, imageDigest
                )
            )
            try:
                logger.debug("archiving analysis data")
                rc = catalog_client.put_document(
                    "analysis_data", imageDigest, image_data
                )
                ...
                event = events.SaveAnalysisFailed(
                    user_id=userId, image_digest=imageDigest, error=err.to_dict()
                )
                analysis_events.append(event)
                raise err
            ...
            if rc:
                try:
                    ...
                    for content_type in all_content_types:
                        try:
                            image_content_data[
                                ...
                                image_data, content_type, manifest=manifest
                            )
                        except Exception as err:
                    ...
                    if image_content_data:
                        ...
                        rc = catalog_client.put_document(
                            "image_content_data", imageDigest, image_content_data
                        )
                ...
                logger.info(
                    "adding image to policy engine: userid={} imageId={} imageDigest={}".format(
                        userId, imageId, imageDigest
                    )
                )
                try:
                    ...
                    for retry_wait in [1, 3, 5, 0]:
                        try:
                            ...
                            logger.debug(
                                "policy engine request: " + image_analysis_fetch_url
                            )
                            resp = pe_client.ingress_image(
                                userId, imageId, image_analysis_fetch_url
                            )
                            logger.debug(
                                "policy engine image add response: " + str(resp)
                            )
                            client_success = True
                            break
                        except Exception as e:
                            logger.warn(
                                "attempt failed, will retry - exception: {}".format(e)
                            )
                            last_exception = e
                    ...
                    event = events.PolicyEngineLoadAnalysisFailed(
                        user_id=userId, image_digest=imageDigest, error=newerr.to_dict()
                    )
                    analysis_events.append(event)
                    raise newerr
                ...
                image_record[
                    "analysis_status"
                ] = anchore_engine.subsys.taskstate.complete_state("analyze")
            ...
            image_record[
                "analysis_status"
            ] = anchore_engine.subsys.taskstate.fault_state("analyze")
            image_record["image_status"] = anchore_engine.subsys.taskstate.fault_state(
                "image_status"
            )
            rc = catalog_client.update_image(imageDigest, image_record)
            ...
            if userId and imageDigest:
                for image_detail in image_record["image_detail"]:
                    ...
                    event = events.UserAnalyzeImageFailed(
                        user_id=userId, full_tag=fulltag, error=str(err)
                    )
                    analysis_events.append(event)
        finally:
            if analysis_events:
            ...
            try:
                ...
                if totalsize > cachemax:
                    ...
                    sorted_layers = sorted(
                        list(layertimes.items()), key=operator.itemgetter(1)
                    )
            ...
            if len(threads) < max_analyze_threads:
                ...
                if qobj:
                    ...
                    athread = threading.Thread(
                        ...
                        args=(system_user_auth, myqobj, layer_cache_enable),
                    )
                    athread.start()
